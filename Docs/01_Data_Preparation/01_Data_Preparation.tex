...
\title{01\_Data\_Preparation: MF4 \texorpdfstring{$\rightarrow$}{->} Cleaned 10\,ms Dataset}
...
We restrict learning to segments where the vehicle is moving and approximately straight:
\begin{itemize}
...
The figures below are generated directly from the canonical cleaned dataset
  	\texttt{Outputs/01\_Data\_Preparation/Aero\_Validation\_Signals\_cleaned\_10ms.csv}.

\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{../Outputs/01_Data_Preparation/plots/cleaned_speed_steering_example.png}
  \caption{Example time window from the cleaned dataset showing GPS speed, wheel speed (converted), and steering angle.}
\end{figure}

\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{../Outputs/01_Data_Preparation/plots/dvdt_vs_imu_long_example.png}
  \caption{Example comparison between $dv/dt$ (computed from GPS speed) and the derived longitudinal IMU acceleration signal \texttt{IMU\_ACCEL\_LONG\_ms2}.}
\end{figure}
