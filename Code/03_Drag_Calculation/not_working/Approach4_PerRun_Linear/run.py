...
def _plot_summary(coeffs: pd.DataFrame, *, plots_dir: Path, config_order: list[str]) -> None:
    ...
    plt.xlabel("Run ID")
    plt.ylabel("CdA [m$^2$]")
    plt.title("CdA per run by configuration (linear v-term)")
    plt.tight_layout()
    plt.savefig(plots_dir / "CdA_per_run_by_config.png", dpi=160)
    plt.close()

    order = [c for c in config_order if c in set(coeffs["config"].astype(str))] if config_order else None
    plt.figure(figsize=(6, 4))
    sns.boxplot(data=coeffs, x="config", y="CdA_m2", order=order)
    plt.xlabel("Configuration")
    plt.ylabel("CdA [m$^2$]")
    plt.title("CdA distribution by configuration (linear v-term)")
    plt.tight_layout()
...
def _plot_run_fit(run_id: int, sl: pd.DataFrame, *, k2: float, k1: float, c: float, plots_dir: Path) -> None:
    ...
    plt.title(f"Coastdown fit: run {run_id:03d} (linear v-term)")
    plt.legend(loc="best")
    plt.tight_layout()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Approach 4: per-run regression including linear v-term (powertrain friction proxy).")
    ...
    parser.add_argument("--out-root", type=Path, default=None, help="Output root dir (default: Outputs/03_Drag_Calculation/Approach4_PerRun_Linear)")
    ...
    out_root = args.out_root if args.out_root is not None else (repo / "Outputs" / "03_Drag_Calculation" / "Approach4_PerRun_Linear")
