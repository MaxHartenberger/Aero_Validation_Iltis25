...
def _plot_cfg_bar(cfg_df: pd.DataFrame, *, plots_dir: Path, config_order: list[str]) -> None:
    ...
    plt.ylabel("CdA [m$^2$]")
    plt.title("CdA by configuration (pooled fit, linear v-term)")
    plt.tight_layout()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Approach 6: pooled-by-config fit including linear v-term (powertrain friction proxy).")
    ...
    parser.add_argument("--out-root", type=Path, default=None, help="Output root dir (default: Outputs/03_Drag_Calculation/Approach6_PooledByConfig_Linear)")
    ...
    out_root = args.out_root if args.out_root is not None else (repo / "Outputs" / "03_Drag_Calculation" / "Approach6_PooledByConfig_Linear")
    ...
    data: dict[str, dict[str, list[np.ndarray]]] = {cfg: {"v2": [], "v": [], "decel": []} for cfg in config_order}
    ...
    for cfg in config_order:
        ...
        mask = np.isfinite(v2) & np.isfinite(v) & np.isfinite(decel)
        v2 = v2[mask]
        v = v[mask]
        decel = decel[mask]
        if v2.size < 50:
        ...
        X = np.column_stack([v2, v, np.ones_like(v2)])
        beta, r2, n_points = robust_linear_fit_multi(X, decel)
        k2, k1, c = beta.tolist()
    ...
    summary_like = pd.DataFrame({"config": cfg_df["config"], "CdA_m2_mean": cfg_df["CdA_m2"]})
