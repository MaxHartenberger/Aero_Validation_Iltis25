...
def _plot_paired_summary(pairs: pd.DataFrame, *, plots_dir: Path, config_order: list[str]) -> None:
    ...
    plt.ylabel("CdA [m$^2$]")
    plt.title("CdA per paired opposite-direction run")
    plt.tight_layout()
    plt.savefig(plots_dir / "CdA_per_pair_by_config.png", dpi=160)
    plt.close()

    if config_order:
        order = [c for c in config_order if c in set(pairs["config"].astype(str))]
    else:
        order = None
    ...
    plt.ylabel("CdA [m$^2$]")
    plt.title("CdA distribution by configuration (paired)")
    plt.tight_layout()
...
def main() -> None:
    ...
    parser.add_argument("--out-root", type=Path, default=None, help="Output root dir (default: Outputs/03_Drag_Calculation/Approach2_PairedOpposites)")
    ...
    out_root = args.out_root if args.out_root is not None else (repo / "Outputs" / "03_Drag_Calculation" / "Approach2_PairedOpposites")
    ...
    # 2) Pair consecutive runs (i, i+1) within same config
